...
def log_validation(
    ...
    with torch.no_grad(), torch.autocast("cuda"):
        ...
        B, C, T, H, W = len(config.validation_lvefs), 4, 64, config.unet.sample_size, config.unet.sample_size
    ...
    with torch.no_grad(): # no autocast
        ...
        ref_frames = rearrange(ref_frames, "b c t h w -> (b t) c h w")
        ...
        ref_frames = rearrange(ref_frames, "(b t) c h w -> b c t h w", b=B)  
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ref_frame, latents, lvef = batch['prior_frames'], batch['target_frames'], batch['lvef']
