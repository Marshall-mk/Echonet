...
class TensorSequenceSet(Dataset):
    def __init__(self, root, seq_len=8):
        """
        Initialize the TensorSet dataset.
        
        Args:
            root (str): Directory containing the .pt tensor files.
            seq_len (int): Number of consecutive tensors to load per sequence (default: 1).
        """
        self.root = root
        self.seq_len = seq_len
        # Get all .pt files and sort them for consistent ordering
        self.all_tensors = glob(os.path.join(root, "*.pt"))
        self.all_tensors.sort()
    ...
    def __len__(self):
        ...
        Returns:
            int: Number of sequences, calculated as max(0, total_tensors - seq_len + 1).
        """
        return max(0, len(self.all_tensors) - self.seq_len + 1)

    def __getitem__(self, idx):
        """
        Load a sequence of tensors starting at the given index.
        
        Args:
            idx (int): Starting index of the sequence.
            
        Returns:
            torch.Tensor: A tensor of shape (seq_len, ...) containing the sequence.
        """
        # Load seq_len consecutive tensors starting at idx
        tensors = [
            torch.load(self.all_tensors[idx + i], map_location='cpu')
...
class ARLatent(Dataset):
    def __init__(self, 
                 ...
                 target_fps=32,
                 target_nframes=64,
                 text_template="An echocardiography video with ejection fraction {}%",
        ...
        assert all([o in ["text", "prior_frame", "target_frame"] for o in self.outputs]),"Outputs must be a list of text, prior_frame, target_frame"
        assert os.path.exists(os.path.join(self.root, self.datafolder)), f"Data folder {os.path.join(self.root, self.datafolder)} does not exist"
        assert os.path.exists(os.path.join(self.root, "FileList.csv")), f"FileList.csv does not exist in {self.root}"
        ...
        # check if videos are reachable
        self.metadata["VideoPath"] = self.metadata["FileName"].apply(lambda x: os.path.join(self.root, self.datafolder, x) if x.endswith(ext) else os.path.join(self.root, self.datafolder, x.split('.')[0] + ext))
        self.metadata["VideoExists"] = self.metadata["VideoPath"].apply(lambda x: os.path.exists(x))
        self.metadata = self.metadata[self.metadata["VideoExists"]]
        self.metadata.reset_index(inplace=True, drop=True)
        if len(self.metadata) == 0:
            raise ValueError(f"No data found in folder {os.path.join(self.root, self.datafolder)}")
    ...
    def _build_sequence_pairs_latent(self):
        ...
        for idx in range(len(self.metadata)):
            ...
            try:
                ...
                min_frames_needed = self.prior_sequence_length + self.stride_between_sequences #+ self.target_sequence_length
                
                if num_frames < min_frames_needed:
                    print(f"Skipping {latent_file}: not enough frames ({num_frames} < {min_frames_needed})")
                    continue
        ...
        print(f"Created {len(self.sequence_pairs)} sequence pairs from {len(self.metadata)} videos")
    ...
    def __getitem__(self, idx):
        ...
        prior_sequence = latent_video_tensor[prior_start_idx:prior_end_idx]  # prior_sequence_length x C x H x W
        target_sequence = latent_video_tensor[target_start_idx:target_end_idx]  # target_sequence_length x C x H x W
        
        # Rearrange to C x T x H x W format expected by 3D models
        prior_sequence = prior_sequence.permute(1, 0, 2, 3)  # C x prior_sequence_length x H x W
        target_sequence = target_sequence.permute(1, 0, 2, 3)  # C x target_sequence_length x H x W
